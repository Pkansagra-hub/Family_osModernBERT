...
Usage:
    python synthetic_embedding_generator_v2.py generate --count 200000
    python synthetic_embedding_generator_v2.py generate --count 200000 --openrouter
    python synthetic_embedding_generator_v2.py stats
"""
...
def _has_google_generation_config() -> bool:
    """Return True when Gemini/Vertex configuration is available."""
    return bool(
...
def _should_use_vertex_ai(explicit_vertex_ai: bool, force_openrouter: bool = False) -> bool:
    """Resolve which backend to use for generation."""
    if force_openrouter:
        return False
    if explicit_vertex_ai or USE_VERTEX_AI:
        return True
    return _has_google_generation_config()
...
MINED_V2_OUTPUT_DIR = BASE_DIR / "data" / "familyos" / "embeddings" / "mined_v2"
GOLD_CANDIDATES_OUTPUT_DIR = BASE_DIR / "data" / "familyos" / "gold_candidates"
FAILURE_CASE_BENCHMARK_OUTPUT_DIR = (
    BASE_DIR / "data" / "familyos" / "benchmarks" / "failure_cases_candidates"
)
UNIFIED_SYNTHETIC_OUTPUT_DIR = BASE_DIR / "data" / "familyos" / "unified" / "output_synthetic"
PROGRESS_FILE = OUTPUT_DIR / "progress.json"
HASH_INDEX_FILE = OUTPUT_DIR / "hash_index.jsonl"

# Processing settings
SAMPLES_PER_REQUEST = 15  # Generate 15 triplets per API call
MAX_STALLED_BATCHES = 3  # Stop workers after repeated zero-progress batches
...
class GenerationMode(Enum):
    ...
    QUERY_DOC = "query_doc"
    WRONG_PERSON_NEGATIVE = "wrong_person_negative"
    WRONG_TIME_NEGATIVE = "wrong_time_negative"
    SAFETY_EMOTION_NEGATIVE = "safety_emotion_negative"
    GOLD_REGENERATION = "gold_regeneration"
    FAILURE_CASE_BENCHMARK = "failure_case_benchmark"
...
QUERY_DOC_SYSTEM_PROMPT = """You are an expert retrieval data generator for FamilyOS. Generate realistic query-document JSON records for FamilyOS retrieval training. Return strict JSON only."""

WRONG_PERSON_NEGATIVE_SYSTEM_PROMPT = """You are an expert FamilyOS hard-negative generator. Create strict JSON triplets where the negative preserves the situation but changes the key person or kinship target."""

WRONG_TIME_NEGATIVE_SYSTEM_PROMPT = """You are an expert FamilyOS hard-negative generator. Create strict JSON triplets where the negative preserves the situation but changes the temporal interpretation."""

SAFETY_EMOTION_NEGATIVE_SYSTEM_PROMPT = """You are an expert FamilyOS hard-negative generator. Create strict JSON triplets where the negative is lexically close but differs in emotionally or safety critical meaning."""

GOLD_REGENERATION_SYSTEM_PROMPT = """You are an expert evaluation-set generator for FamilyOS. Produce strict JSON candidates suitable for human review when regenerating gold retrieval examples."""

FAILURE_CASE_BENCHMARK_SYSTEM_PROMPT = """You are an expert adversarial benchmark generator for FamilyOS. Produce strict JSON records that capture realistic failure cases and regression targets."""

SYSTEM_PROMPTS: dict[GenerationMode, str] = {
    GenerationMode.CROSS_CLUSTER: SYSTEM_PROMPT,
    GenerationMode.HARD_NEGATIVE: HARD_NEGATIVE_SYSTEM_PROMPT,
    GenerationMode.QUERY_DOC: QUERY_DOC_SYSTEM_PROMPT,
    GenerationMode.WRONG_PERSON_NEGATIVE: WRONG_PERSON_NEGATIVE_SYSTEM_PROMPT,
    GenerationMode.WRONG_TIME_NEGATIVE: WRONG_TIME_NEGATIVE_SYSTEM_PROMPT,
    GenerationMode.SAFETY_EMOTION_NEGATIVE: SAFETY_EMOTION_NEGATIVE_SYSTEM_PROMPT,
    GenerationMode.GOLD_REGENERATION: GOLD_REGENERATION_SYSTEM_PROMPT,
    GenerationMode.FAILURE_CASE_BENCHMARK: FAILURE_CASE_BENCHMARK_SYSTEM_PROMPT,
}

SUPPORTED_GENERATE_MODES = {
    GenerationMode.CROSS_CLUSTER,
    GenerationMode.HARD_NEGATIVE,
    GenerationMode.QUERY_DOC,
    GenerationMode.WRONG_PERSON_NEGATIVE,
    GenerationMode.WRONG_TIME_NEGATIVE,
    GenerationMode.SAFETY_EMOTION_NEGATIVE,
}

MODE_OUTPUT_REGISTRY: dict[GenerationMode, dict[str, object]] = {
    GenerationMode.CROSS_CLUSTER: {
        "output_dir": OUTPUT_DIR,
        "shard_prefix": "triplets",
        "display_name": "CROSS-CLUSTER (silver_synthetic)",
        "manifest_name": "manifest.json",
        "implemented": True,
    },
    GenerationMode.HARD_NEGATIVE: {
        "output_dir": HARD_NEG_OUTPUT_DIR,
        "shard_prefix": "triplets",
        "display_name": "HARD NEGATIVES (hard_negatives)",
        "manifest_name": "manifest.json",
        "implemented": True,
    },
    GenerationMode.QUERY_DOC: {
        "output_dir": MINED_V2_OUTPUT_DIR / "query_doc",
        "shard_prefix": "query_doc_pairs",
        "display_name": "QUERY-DOC (mined_v2/query_doc)",
        "manifest_name": "manifest.json",
        "implemented": True,
    },
    GenerationMode.WRONG_PERSON_NEGATIVE: {
        "output_dir": MINED_V2_OUTPUT_DIR / "wrong_person",
        "shard_prefix": "wrong_person_negatives",
        "display_name": "WRONG-PERSON NEGATIVES (mined_v2/wrong_person)",
        "manifest_name": "manifest.json",
        "implemented": True,
    },
    GenerationMode.WRONG_TIME_NEGATIVE: {
        "output_dir": MINED_V2_OUTPUT_DIR / "wrong_time",
        "shard_prefix": "wrong_time_negatives",
        "display_name": "WRONG-TIME NEGATIVES (mined_v2/wrong_time)",
        "manifest_name": "manifest.json",
        "implemented": True,
    },
    GenerationMode.SAFETY_EMOTION_NEGATIVE: {
        "output_dir": MINED_V2_OUTPUT_DIR / "safety_emotion",
        "shard_prefix": "safety_emotion_negatives",
        "display_name": "SAFETY/EMOTION NEGATIVES (mined_v2/safety_emotion)",
        "manifest_name": "manifest.json",
        "implemented": True,
    },
    GenerationMode.GOLD_REGENERATION: {
        "output_dir": GOLD_CANDIDATES_OUTPUT_DIR,
        "shard_prefix": "gold_candidates",
        "display_name": "GOLD CANDIDATES",
        "manifest_name": "manifest.json",
        "implemented": False,
    },
    GenerationMode.FAILURE_CASE_BENCHMARK: {
        "output_dir": FAILURE_CASE_BENCHMARK_OUTPUT_DIR,
        "shard_prefix": "failure_case_candidates",
        "display_name": "FAILURE CASE BENCHMARK CANDIDATES",
        "manifest_name": "manifest.json",
        "implemented": False,
    },
}

MODE_SCHEMA_REGISTRY: dict[GenerationMode, dict[str, list[str]]] = {
    GenerationMode.CROSS_CLUSTER: {
        "required_fields": [
            "anchor",
            "positive",
    ...
    GenerationMode.QUERY_DOC: {
        ...
        "optional_fields": ["generator_rationale"],
    },
    GenerationMode.WRONG_PERSON_NEGATIVE: {
        "required_fields": [
            "mode",
    ...
    GenerationMode.SAFETY_EMOTION_NEGATIVE: {
        ...
        "optional_fields": [
            "generator_rationale",
            "safety_label_anchor",
            "safety_label_negative",
            "emotion_label_anchor",
            "emotion_label_negative",
        ],
    },
    GenerationMode.GOLD_REGENERATION: {
        "required_fields": [
...
def get_mode_output_config(mode: GenerationMode) -> dict[str, object]:
    """Return output routing metadata for a generation mode."""
    return MODE_OUTPUT_REGISTRY[mode]


def get_mode_schema(mode: GenerationMode) -> dict[str, list[str]]:
    """Return the required and optional fields for a generation mode."""
    return MODE_SCHEMA_REGISTRY[mode]


def get_mode_system_prompt(mode: GenerationMode) -> str:
    """Return the generation prompt for a mode."""
    return SYSTEM_PROMPTS[mode]


def _safe_tasks(sample: dict) -> dict:
    """Return the tasks object from a unified sample."""
    tasks = sample.get("tasks", {})
    return tasks if isinstance(tasks, dict) else {}


def _safe_hub_routing(sample: dict) -> dict:
    """Return the hub_routing object from a unified sample."""
    hub_routing = sample.get("hub_routing", {})
    return hub_routing if isinstance(hub_routing, dict) else {}


def _extract_family_entities(sample: dict) -> list[dict]:
    """Extract family/person entities from a unified sample."""
    tasks = _safe_tasks(sample)
    entities = tasks.get("ner_family", [])
    if not isinstance(entities, list):
        return []
    return [
        entity
        for entity in entities
        if isinstance(entity, dict) and entity.get("label") in {"PERSON", "KINSHIP"}
    ]


def _candidate_has_family_signal(sample: dict) -> bool:
    """Check whether a unified sample contains family relationship/person signal."""
    tasks = _safe_tasks(sample)
    relations = tasks.get("relations", [])
    return bool(_extract_family_entities(sample) or relations)


def _candidate_has_temporal_signal(sample: dict) -> bool:
    """Check whether a unified sample contains temporal signal."""
    tasks = _safe_tasks(sample)
    temporal = tasks.get("temporal", [])
    return isinstance(temporal, list) and len(temporal) > 0


def _candidate_has_safety_emotion_signal(sample: dict) -> bool:
    """Check whether a unified sample contains emotion or safety signal."""
    tasks = _safe_tasks(sample)
    emotions = tasks.get("emotions", [])
    safety = tasks.get("safety_familyos", "GREEN")
    hub_routing = _safe_hub_routing(sample)
    has_non_neutral_emotion = bool(
        isinstance(emotions, list)
        and [emotion for emotion in emotions if emotion and emotion != "neutral"]
    )
    return safety in {"AMBER", "RED", "CRISIS"} or has_non_neutral_emotion or bool(
        hub_routing.get("EMO")
    )


def _project_source_candidate(sample: dict) -> dict:
    """Project a unified sample to the prompt fields needed by advanced modes."""
    tasks = _safe_tasks(sample)
    projected_tasks = {
        "emotions": tasks.get("emotions", []),
        ...
        "ner_family": tasks.get("ner_family", []),
        "relations": tasks.get("relations", []),
        "temporal": tasks.get("temporal", []),
    }
    return {
...
def build_slice_tags_from_sources(mode: GenerationMode, source_batch: list[dict]) -> list[str]:
    """Build deterministic slice tags from source samples."""
    ...
    for sample in source_batch:
        ...
        if ingress:
            tags.add(f"ingress:{str(ingress).lower()}")
        if intent:
            tags.add(f"intent:{str(intent).lower()}")
        if safety:
            tags.add(f"safety:{str(safety).lower()}")
        if hub_routing.get("EMO"):
            tags.add("hub:emo")
        if hub_routing.get("REL"):
            tags.add("hub:rel")
        if hub_routing.get("MEM"):
            tags.add("hub:mem")
        if hub_routing.get("TASK"):
            tags.add("hub:task")
        if _candidate_has_temporal_signal(sample):
            tags.add("has:temporal")
        if _candidate_has_family_signal(sample):
            tags.add("has:family_signal")
        if _candidate_has_safety_emotion_signal(sample):
            tags.add("has:safety_emotion_signal")

    return sorted(tags)


def _mode_default_difficulty(mode: GenerationMode) -> str:
    """Return the default difficulty level for a generation mode."""
    if mode in {GenerationMode.WRONG_PERSON_NEGATIVE, GenerationMode.WRONG_TIME_NEGATIVE}:
        return "hard"
    if mode == GenerationMode.SAFETY_EMOTION_NEGATIVE:
        return "very_hard"
    return "medium"


class CandidateSourceAdapter:
    """Base class for advanced generation source adapters."""

    def __init__(self, mode: GenerationMode, source_name: str, source_limit: int | None = None):
        self.mode = mode
        ...
        if not self.candidates:
            raise ValueError(
                f"No source candidates available for mode '{mode.value}' from '{source_name}'"
            )
    ...
    def get_batch(self, batch_size: int) -> list[dict]:
        """Return a deterministic rotating batch of source candidates."""
        with self.lock:
            batch = []
            while len(batch) < batch_size:
                candidate = self.candidates[self._cursor % len(self.candidates)]
                batch.append(candidate)
                self._cursor += 1
            return batch


class UnifiedOutputAdapter(CandidateSourceAdapter):
    """Reads unified synthetic JSONL shards and filters candidates by mode."""

    def __init__(self, mode: GenerationMode, source_limit: int | None = None):
        self.source_dir = UNIFIED_SYNTHETIC_OUTPUT_DIR
        super().__init__(mode=mode, source_name="unified_output_synthetic", source_limit=source_limit)

    def _accept_candidate(self, sample: dict) -> bool:
        if not sample.get("text"):
            return False

        if self.mode == GenerationMode.WRONG_PERSON_NEGATIVE:
            return _candidate_has_family_signal(sample)

        if self.mode == GenerationMode.WRONG_TIME_NEGATIVE:
            return _candidate_has_temporal_signal(sample)

        if self.mode == GenerationMode.QUERY_DOC:
            return len(sample.get("text", "")) >= 12

        if self.mode == GenerationMode.SAFETY_EMOTION_NEGATIVE:
            return _candidate_has_safety_emotion_signal(sample)
    ...
    def _load_candidates(self) -> list[dict]:
        ...
        for shard in sorted(self.source_dir.glob("shard_*.jsonl")):
            with open(shard, encoding="utf-8") as f:
                for line in f:
                    ...
                    sample["_source_file"] = str(shard)
                    scanned += 1
                    if self._accept_candidate(sample):
                        candidates.append(sample)

                    if self.source_limit is not None and len(candidates) >= self.source_limit:
                        self._rng.shuffle(candidates)
                        logger.info(
                            f"Loaded {len(candidates)} source candidates for {self.mode.value} "
                            f"after scanning {scanned} unified rows"
                        )
                        return candidates
...
def create_candidate_source_adapter(
    mode: GenerationMode,
    input_source: str,
    source_limit: int | None = None,
) -> CandidateSourceAdapter | None:
    """Create a source adapter for advanced modes."""
    if mode in {GenerationMode.CROSS_CLUSTER, GenerationMode.HARD_NEGATIVE}:
        return None

    if input_source == "unified_output_synthetic":
        return UnifiedOutputAdapter(mode=mode, source_limit=source_limit)

    raise ValueError(f"Unsupported input source: {input_source}")


def build_advanced_mode_user_prompt(
    mode: GenerationMode,
    source_batch: list[dict],
    num_records: int,
) -> str:
    """Build a mode-specific prompt using source unified samples."""
    source_json = "\n".join(
        json.dumps(_project_source_candidate(sample), ensure_ascii=False) for sample in source_batch
    )

    if mode == GenerationMode.WRONG_PERSON_NEGATIVE:
        mode_instructions = """For each source sample, create exactly one hard negative triplet.
- Keep the same situation, event, and timing.
- Keep the anchor faithful to the source sample.
- Make the positive a close paraphrase of the anchor.
- Make the negative change WHO the event is about: swap the person, kinship target, or actor.
- Use hard_negative_type = entity_swap.
- mismatch_features should explain the person swap in short phrases.
- difficulty should be hard.
- Return strict JSONL only."""
    elif mode == GenerationMode.WRONG_TIME_NEGATIVE:
        mode_instructions = """For each source sample, create exactly one hard negative triplet.
- Keep the same people and same core event.
- Keep the anchor faithful to the source sample.
- Make the positive a close paraphrase of the anchor.
- Make the negative change WHEN the event happened, its recurrence, chronology, or temporal framing.
- Use hard_negative_type = temporal_shift.
- mismatch_features should explain the temporal mismatch in short phrases.
- difficulty should be hard.
- Return strict JSONL only."""
    elif mode == GenerationMode.QUERY_DOC:
        mode_instructions = """For each source sample, create exactly one retrieval pair.
- query should sound like something a FamilyOS user would type or ask.
- document should sound like a canonical memory, note, or fact record that matches the query.
- pair_type must be exactly one of: semantic_match, event_match, reminder_match, memory_match. Do not invent other types.
- shared_features should be a short list of 2-5 retrieval cues.
- difficulty should be easy, medium, or hard.
- Return strict JSONL only."""
    elif mode == GenerationMode.SAFETY_EMOTION_NEGATIVE:
        mode_instructions = """For each source sample, create exactly one hard negative triplet.
- Keep the same general topic and lexical neighborhood.
- Keep the anchor faithful to the source sample.
- Make the positive a close paraphrase of the anchor.
- Make the negative a meaningful safety or emotion near miss.
- Use hard_negative_type = safety_emotion_near_miss.
- mismatch_features should explain what emotional or safety meaning changed.
- safety_label_anchor and safety_label_negative must each be a single string (GREEN, AMBER, RED, or CRISIS). Do NOT return a list.
- emotion_label_anchor and emotion_label_negative must each be a single emotion word (e.g. worry, frustration, joy, anger, sadness, neutral). Do NOT return a list.
- difficulty should be very_hard.
- Return strict JSONL only."""
    else:
        raise ValueError(f"Unsupported advanced mode prompt build: {mode.value}")

    return f"""Generate exactly {num_records} records for mode {mode.value}.
...
class SyntheticDataManager:
                    ...
                    with open(shard, encoding="utf-8") as f:
                        for line in f:
                            try:
                                ...
                                triplet_hash = self._compute_record_hash(triplet)
    ...
    def _compute_record_hash(self, record: dict) -> str:
        """Compute a stable dedup hash for any supported generated record."""
        if all(field in record for field in ["anchor", "positive", "negative"]):
            dedup_key = "\t".join(
                [
                    str(record.get("anchor", "")).lower().strip(),
                    str(record.get("positive", "")).lower().strip(),
                    str(record.get("negative", "")).lower().strip(),
                ]
            )
        elif all(field in record for field in ["query", "document"]):
            dedup_key = "\t".join(
                [
                    str(record.get("query", "")).lower().strip(),
                    str(record.get("document", "")).lower().strip(),
                ]
            )
        elif all(field in record for field in ["query", "positive", "negative"]):
            dedup_key = "\t".join(
                [
                    str(record.get("query", "")).lower().strip(),
                    str(record.get("positive", "")).lower().strip(),
            ...
            dedup_key = json.dumps(record, sort_keys=True, ensure_ascii=False)
        return hashlib.md5(dedup_key.encode()).hexdigest()

    def _get_shard_path(self, shard_id: int) -> Path:
        return self.output_dir / f"{self.shard_prefix}_{shard_id:04d}.jsonl"
    ...
    def _get_next_shard_id(self) -> int:
        existing = list(self.output_dir.glob(f"{self.shard_prefix}_*.jsonl"))
        if not existing:
            return 0
        max_id = max(int(p.stem.rsplit("_", 1)[1]) for p in existing)
        if self._count_shard_triplets(max_id) >= self.shard_size:
            return max_id + 1
        return max_id

    def add_triplets(self, triplets: list[dict]) -> int:
        """Add generated records to output with mode-agnostic deduplication."""
        added = 0
        ...
        with self.lock:
            for triplet in triplets:
                ...
                if self.current_shard_count >= self.shard_size:
                    ...
                    logger.info(
                        f"Started new shard: {self.shard_prefix}_{self.current_shard_id:04d}"
                    )
    ...
    def count_total_on_disk(self) -> int:
        """Count total records across all data shards on disk."""
        total = 0
        for shard in self.output_dir.glob(f"{self.shard_prefix}_*.jsonl"):
            try:
                with open(shard, encoding="utf-8") as f:
    ...
    def write_manifest(self, manifest_payload: dict) -> Path:
...
VALID_HARD_NEGATIVE_TYPES = {
    ...
    "quantifier_change",
    "causality_flip",
    "safety_emotion_near_miss",
}


def validate_required_fields(record: dict, mode: GenerationMode) -> tuple[bool, str]:
    """Validate schema-required fields for a record in a specific generation mode."""
...
def validate_triplet(
    ...
    is_valid, error = validate_required_fields(triplet, mode)
    if not is_valid:
        return False, error
...
def _validate_text_field(record: dict, field: str) -> tuple[bool, str]:
    """Validate a text field for generated records."""
    value = record.get(field)
    if not isinstance(value, str):
        return False, f"{field} must be a string"
    if len(value.strip()) < 5:
        return False, f"{field} too short: {len(value.strip())} chars"
    if len(value.strip()) > 1000:
        return False, f"{field} too long: {len(value.strip())} chars"
    return True, ""


def _validate_label_field(record: dict, field: str) -> tuple[bool, str]:
    """Validate a short categorical or identifier field."""
    value = record.get(field)
    if not isinstance(value, str):
        return False, f"{field} must be a string"
...
def _validate_list_field(record: dict, field: str) -> tuple[bool, str]:
    """Validate a list field for generated records."""
    value = record.get(field)
...
def validate_generated_record(record: dict, mode: GenerationMode) -> tuple[bool, str]:
    """Validate a generated record for new v2 modes."""
    is_valid, error = validate_required_fields(record, mode)
    if not is_valid:
        return False, error

    for field in ["source_ids", "source_files", "slice_tags"]:
        is_valid, error = _validate_list_field(record, field)
        if not is_valid:
            return False, error

    if not isinstance(record.get("generator_model"), str):
        return False, "generator_model must be a string"

    if mode == GenerationMode.QUERY_DOC:
        for field in ["query", "document"]:
            is_valid, error = _validate_text_field(record, field)
            if not is_valid:
                return False, error
        for field in ["query_id", "document_id", "pair_type", "difficulty"]:
            is_valid, error = _validate_label_field(record, field)
            if not is_valid:
                return False, error
        is_valid, error = _validate_list_field(record, "shared_features")
        if not is_valid:
            return False, error
        return True, ""
    ...
    if mode in {
        ...
        for field in ["anchor", "positive", "negative"]:
            ...
            if not is_valid:
                return False, error
        is_valid, error = _validate_label_field(record, "hard_negative_type")
        if not is_valid:
            return False, error
        is_valid, error = _validate_list_field(record, "mismatch_features")
        if not is_valid:
            return False, error
        if mode != GenerationMode.SAFETY_EMOTION_NEGATIVE:
            is_valid, error = _validate_label_field(record, "difficulty")
            if not is_valid:
                return False, error
        if record.get("hard_negative_type") not in VALID_HARD_NEGATIVE_TYPES:
            return False, f"Invalid hard_negative_type: {record.get('hard_negative_type')}"
        if mode == GenerationMode.SAFETY_EMOTION_NEGATIVE:
            for field in [
                "safety_label_anchor",
                "safety_label_negative",
    ...
    return False, f"Unsupported validation mode: {mode.value}"


def _normalize_list(value: object) -> list[str]:
    """Normalize list-like metadata fields into a list of strings.

    Handles native lists, stringified Python lists (e.g. "['a', 'b']"),
    stringified JSON arrays, and plain scalar strings.
    """
...
def _normalize_label(value: object, fallback: str = "unspecified") -> str:
    """Normalize a label value to a single clean string.

    Handles native lists (takes first element), stringified Python/JSON
    lists, and plain strings.  Falls back to *fallback* when empty.
    """
    if isinstance(value, list):
...
def _build_query_or_document_id(prefix: str, text: str) -> str:
    """Build a stable ID for a generated text field."""
    digest = hashlib.md5(text.strip().lower().encode()).hexdigest()[:12]
    return f"{prefix}_{digest}"


def postprocess_generated_record(
    record: dict,
    mode: GenerationMode,
    source_batch: list[dict],
    model_name: str,
    ...
    """Fill deterministic metadata defaults for new v2 modes.

    Args:
        record: The raw parsed record from the LLM.
        mode: The generation mode.
        source_batch: All source samples sent in this batch.
        model_name: The model name used for generation.
        source_index: If provided, maps this record to a specific source
            sample for per-record provenance instead of batch-level.
    """
    # Per-record source attribution when index is available
    if source_index is not None and source_index < len(source_batch):
        attribution_sources = [source_batch[source_index]]
    else:
        attribution_sources = source_batch

    record["mode"] = mode.value
    record["source_ids"] = _normalize_list(record.get("source_ids")) or [
        str(sample.get("id", "")) for sample in attribution_sources if sample.get("id")
    ]
    record["source_files"] = _normalize_list(record.get("source_files")) or [
        str(sample.get("_source_file", "")) for sample in attribution_sources if sample.get("_source_file")
    ]
    record["generator_model"] = str(record.get("generator_model") or model_name)
    record["slice_tags"] = _normalize_list(record.get("slice_tags")) or build_slice_tags_from_sources(
        mode, source_batch
    )

    if mode == GenerationMode.WRONG_PERSON_NEGATIVE:
        record["hard_negative_type"] = str(record.get("hard_negative_type") or "entity_swap")
        record["mismatch_features"] = _normalize_list(record.get("mismatch_features")) or [
            "person_swap",
            "kinship_target_change",
        ]
        record["difficulty"] = str(record.get("difficulty") or _mode_default_difficulty(mode))

    elif mode == GenerationMode.WRONG_TIME_NEGATIVE:
        record["hard_negative_type"] = str(record.get("hard_negative_type") or "temporal_shift")
        record["mismatch_features"] = _normalize_list(record.get("mismatch_features")) or [
            "temporal_reference_change",
            "chronology_shift",
        ]
        record["difficulty"] = str(record.get("difficulty") or _mode_default_difficulty(mode))
        ...
        record["pair_type"] = str(record.get("pair_type") or "semantic_match")
        record["shared_features"] = _normalize_list(record.get("shared_features")) or [
            "family_context",
            "semantic_overlap",
        ]
        record["difficulty"] = str(record.get("difficulty") or _mode_default_difficulty(mode))
        record["query_id"] = str(record.get("query_id") or _build_query_or_document_id("qry", query))
        record["document_id"] = str(
            record.get("document_id") or _build_query_or_document_id("doc", document)
        )

    elif mode == GenerationMode.SAFETY_EMOTION_NEGATIVE:
        tasks = _safe_tasks(source_batch[0]) if source_batch else {}
        emotions = tasks.get("emotions", []) if isinstance(tasks.get("emotions", []), list) else []
        source_primary_emotion = emotions[0] if emotions else "neutral"
        source_safety = str(tasks.get("safety_familyos", "GREEN"))

        record["hard_negative_type"] = str(
            record.get("hard_negative_type") or "safety_emotion_near_miss"
        )
        record["mismatch_features"] = _normalize_list(record.get("mismatch_features")) or [
            "emotion_shift",
            "safety_band_shift",
        ...
        record["difficulty"] = str(record.get("difficulty") or _mode_default_difficulty(mode))
        record["safety_label_anchor"] = _normalize_label(
            record.get("safety_label_anchor"), fallback=source_safety
        )
        record["safety_label_negative"] = _normalize_label(
            record.get("safety_label_negative"), fallback="unspecified"
        )
        record["emotion_label_anchor"] = _normalize_label(
            record.get("emotion_label_anchor"), fallback=source_primary_emotion
        )
        record["emotion_label_negative"] = _normalize_label(
            record.get("emotion_label_negative"), fallback="unspecified"
        )
...
def parse_generation_response(
    response_text: str,
    mode: GenerationMode,
    source_batch: list[dict] | None = None,
    model_name: str = "",
) -> list[dict]:
    """Parse and validate mode-specific JSONL generation output."""
    valid_records = []
    ...
    for line in lines:
        ...
        try:
            ...
            if mode in {GenerationMode.CROSS_CLUSTER, GenerationMode.HARD_NEGATIVE}:
                is_valid, error = validate_triplet(record, mode=mode)
            else:
                record = postprocess_generated_record(
                    record, mode, source_batch, model_name,
                    source_index=source_mapping_index,
                )
                is_valid, error = validate_generated_record(record, mode)

            source_mapping_index += 1

            if is_valid:
                valid_records.append(record)
            else:
                logger.debug(f"Invalid generated record: {error}")
...
def parse_triplet_response(response_text: str) -> list[dict]:
    ...
    return parse_generation_response(response_text, mode=_GENERATION_MODE)
...
class EmbeddingTripletGenerator:
    ...
    def __init__(
        ...
        num_parallel: int = 1,
        mode: GenerationMode = GenerationMode.CROSS_CLUSTER,
        input_source: str = "unified_output_synthetic",
        source_limit: int | None = None,
    ):
        ...
        self.source_adapter = create_candidate_source_adapter(mode, input_source, source_limit)
        self.use_vertex_ai = _should_use_vertex_ai(use_vertex_ai, force_openrouter)

        if self.use_vertex_ai:
            project_id = gcp_project_id or GCP_PROJECT_ID
            if not project_id and not _has_google_generation_config():
                raise ValueError(
                    "Gemini backend requested but no Google configuration was found. "
                    "Set GOOGLE_API_KEY, GOOGLE_CLOUD_API_KEY, GOOGLE_APPLICATION_CREDENTIALS, "
                    "or GCP_PROJECT_ID."
                )
            ...
            logger.info(f"Using Gemini/Vertex AI with {num_parallel} parallel worker(s)")
        else:
        ...
        mode_config = get_mode_output_config(mode)
        self.output_manager = SyntheticDataManager(
            output_dir=mode_config["output_dir"],
            shard_prefix=str(mode_config["shard_prefix"]),
            manifest_name=str(mode_config["manifest_name"]),
        )
    ...
    def _drain_stats_queue(self, stats_queue: Queue, stats: dict) -> None:
        """Drain queued worker stats into the aggregate run stats."""
        while not stats_queue.empty():
            batch_stats = stats_queue.get_nowait()
            stats["generated_triplets"] += batch_stats["generated"]
            stats["accepted_triplets"] += batch_stats["added"]

    def _get_client_model_name(self, client) -> str:
        """Return the effective model name for a generation client."""
        return str(getattr(client, "model_name", MODEL))

    def _generate_batch(self, client, user_prompt: str) -> dict[str, int]:
        """Generate one batch of embedding triplets using worker-specific prompt."""
        batch_id = self._get_next_batch_id()
        model_name = self._get_client_model_name(client)
        source_batch: list[dict] | None = None

        try:
            if self.source_adapter is not None:
                source_batch = self.source_adapter.get_batch(self.triplets_per_request)
                logger.info(
                    f"[Worker {client.key_id}] Batch {batch_id}: "
                    f"sending {len(source_batch)} source samples and requesting "
                    f"{self.triplets_per_request} records"
                )
                user_prompt = build_advanced_mode_user_prompt(
                    self.mode,
                    source_batch,
                    self.triplets_per_request,
                )
            ...
            triplets = parse_generation_response(
                response,
                mode=self.mode,
    ...
    def _worker(
        ...
        # Advanced modes build prompts from source batches inside _generate_batch.
        user_prompt = ""
        ...
        if user_prompt:
            ...
            logger.debug(f"[Worker {worker_id}] Starting advanced mode worker")
        ...
        while not stop_event.is_set() and triplets_generated < target_triplets:
            try:
                ...
                if (
                    self.source_adapter is None
                    and worker_id == 0
                    and batch_count > 0
                    and batch_count % refresh_interval == 0
                ):
                    logger.info(
                ...
                # Get potentially updated prompt for legacy modes.
                if self.source_adapter is None:
                    user_prompt = get_worker_user_prompt(worker_id, self.triplets_per_request)

                batch_result = self._generate_batch(client, user_prompt)
                ...
                if added == 0:
                    stalled_batches += 1
                    logger.warning(
                        f"[Worker {worker_id}] No progress on batch {batch_count} "
                        f"({stalled_batches}/{MAX_STALLED_BATCHES} stalled batches)"
                    )
                    if stalled_batches >= MAX_STALLED_BATCHES:
                        logger.warning(
                            f"[Worker {worker_id}] Stopping after {MAX_STALLED_BATCHES} "
                            "consecutive zero-add batches to avoid wasting API calls"
                        )
                        break
                else:
                    stalled_batches = 0
    ...
    def run(self, target_triplets: int = 10000) -> dict:
        ...
        with ThreadPoolExecutor(max_workers=len(self.clients)) as executor:
            ...
            try:
                while not all(f.done() for f in futures):
                    self._drain_stats_queue(stats_queue, stats)
                    time.sleep(1)
        ...
        self.output_manager.flush_hashes()
        final_stats = self.output_manager.get_stats()
        stats.update(final_stats)
        stats["accepted_triplets"] = final_stats["total_triplets"]
        stats["end_time"] = datetime.now().isoformat()
        stats["duration_minutes"] = (datetime.now() - start_time).total_seconds() / 60

        model_name = self._get_client_model_name(self.clients[0])
        disk_total = self.output_manager.count_total_on_disk()
        manifest_payload = {
            "mode": self.mode.value,
            "generator_model": model_name,
            "input_source": self.input_source,
            "source_candidate_count": (
                len(self.source_adapter.candidates) if self.source_adapter is not None else None
            ),
            "system_prompt_registered": True,
            "target_triplets": target_triplets,
            "generated_triplets_this_run": stats["generated_triplets"],
            "accepted_triplets_this_run": final_stats["total_triplets"],
            "total_records_on_disk": disk_total,
            "duplicates_skipped": final_stats["duplicates_skipped"],
            "output_dir": str(self.output_manager.output_dir),
            "shard_prefix": self.output_manager.shard_prefix,
            "shard_count": final_stats["shard_count"],
            "start_time": stats["start_time"],
            "end_time": stats["end_time"],
            "duration_minutes": stats["duration_minutes"],
        }
        manifest_path = self.output_manager.write_manifest(manifest_payload)
        stats["manifest_path"] = str(manifest_path)

        logger.info(f"\n{'='*60}")
        logger.info("GENERATION COMPLETE")
        logger.info(f"This run: {final_stats['total_triplets']:,} accepted")
        logger.info(f"Total on disk: {disk_total:,}")
...
def show_stats():
    ...
    for mode in sorted(SUPPORTED_GENERATE_MODES, key=lambda item: item.value):
        config = get_mode_output_config(mode)
        label = str(config["display_name"])
        ...
        if any("anchor_cluster" in record for record in all_triplets):
        ...
        if any("pair_type" in record for record in all_triplets):
            pair_types = Counter(t.get("pair_type", "") for t in all_triplets)
            print("\nPair Type Distribution:")
            for pair_type, count in pair_types.most_common():
                print(f"  {pair_type:25s} {count:6,} ({100*count/total:5.1f}%)")
...
def main():
    ...
    gen_parser.add_argument(
        "--mode",
        type=str,
        choices=[mode.value for mode in sorted(SUPPORTED_GENERATE_MODES, key=lambda item: item.value)],
        default="cross_cluster",
        help=(
            "Generation mode: cross_cluster, hard_negative, wrong_person_negative, "
            "wrong_time_negative, query_doc, or safety_emotion_negative"
        ),
    )
    gen_parser.add_argument(
        "--input-source",
        type=str,
        choices=["unified_output_synthetic"],
        default="unified_output_synthetic",
        help="Input source for advanced modes",
    )
    gen_parser.add_argument(
        "--source-limit",
        type=int,
        default=None,
        help="Optional max number of source candidates to load for advanced modes",
    )
    ...
    gen_parser.add_argument(
        "--vertex-ai",
        action="store_true",
        help="Force Gemini/Vertex AI backend",
    )
    gen_parser.add_argument(
        "--openrouter",
        action="store_true",
        help="Force OpenRouter backend instead of auto-selecting Gemini",
    )
    ...
    if args.command == "generate":
        if args.vertex_ai and args.openrouter:
            parser.error("Use either --vertex-ai or --openrouter, not both")
        ...
        resolved_use_vertex_ai = _should_use_vertex_ai(args.vertex_ai, args.openrouter)
        if resolved_use_vertex_ai:
            if args.vertex_ai:
                logger.info("Backend: Gemini/Vertex AI (forced by CLI)")
            elif USE_VERTEX_AI:
                logger.info("Backend: Gemini/Vertex AI (USE_VERTEX_AI=true)")
            else:
                logger.info("Backend: Gemini/Vertex AI (auto-selected from available Google config)")
        else:
            logger.info("Backend: OpenRouter")
        ...
        if mode == GenerationMode.HARD_NEGATIVE:
            ...
            logger.info(
                f"Advanced mode: using input source '{args.input_source}' "
                f"with source_limit={args.source_limit}"
            )

        generator = EmbeddingTripletGenerator(
            triplets_per_request=args.triplets_per_request,
            delay_between_requests=delay,
            use_vertex_ai=resolved_use_vertex_ai,
            force_openrouter=args.openrouter,
