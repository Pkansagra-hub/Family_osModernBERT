...
OPENROUTER_API_KEY = "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"
...
SAMPLES_PER_REQUEST = 500  # Ask for 50 samples per API call (free tier limit)
...
You MUST follow this label schema from the FamilyOS Family NER README:

### LABEL SCHEMA (from README.md)
Use only these integer IDs:

0  → O
1  → B-PERSON        2  → I-PERSON        # Named individuals (Emma, John Smith, Sarah)
3  → B-KINSHIP       4  → I-KINSHIP       # Family relationship terms (mom, dad, didi, nana, bhai, chacha, etc.)
5  → B-NICKNAME      6  → I-NICKNAME      # Cute family nicknames (Panda, Bunny, Sweetie, Baby Bear, etc.)
7  → B-PET           8  → I-PET           # Pet names and references (Max, Whiskers, our dog, golden retriever Max, etc.)
9  → B-HOME_LOC     10  → I-HOME_LOC      # Locations within or around home (kitchen, Emma's room, backyard, garden, garage, balcony, etc.)
11 → B-FAMILY_EVENT 12  → I-FAMILY_EVENT  # One-time family occasions (birthday party, anniversary dinner, graduation ceremony, wedding, etc.)
13 → B-ROUTINE      14  → I-ROUTINE       # Regular activities (school run, dinner time, bedtime story, morning walk, etc.)
15 → B-TRADITION    16  → I-TRADITION     # Recurring family rituals (Sunday brunch, movie night, Diwali celebration, Christmas Eve dinner, etc.)
17 → B-MILESTONE    18  → I-MILESTONE     # Significant life events (first steps, graduation day, lost first tooth, first word, etc.)
19 → B-HEIRLOOM     20  → I-HEIRLOOM      # Sentimental family objects (grandma's necklace, dad's watch, family photo album, etc.)

Do NOT invent any new labels or IDs. Use ONLY 0–20 as defined above.

### STYLE & CONTENT REQUIREMENTS (VERY IMPORTANT)

You are generating training data for FamilyOS. The sentences should look like real things families write or say:

- Use **multi-sentence examples** in about 10–20% of samples.
  - Example: "Panda took her first steps in the living room today. Mom called Grandma right away to tell her."
- Use **casual family speech** (natural, warm, sometimes slightly messy).
- Use **temporal markers** frequently (but DO NOT tag them):
  - "today", "tonight", "tomorrow", "on Sunday", "after dinner", "before school", "this weekend", etc.
- Use **mild Indian-English code mixing** where natural, especially for kinship and festivals:
  - "didi", "bhai", "nana", "dada", "chacha", "mummy", "papa", "Diwali", "Holi", etc.
- Include **context-rich examples**:
  - short diary entries, notes, reminders, text messages, conversations.
- Include **reminder-style lines**:
  - "Pick up Emma from school after piano class."
  - "Reminder: Nana's birthday party at home tomorrow night."
- Include **diary-style lines**:
  - "Today was special because Emma took her first steps in the kitchen while Mom and Dad watched."
- Include **natural dialogue snippets**:
  - "Mom told Bhai to help with the school run while Dad finished cooking dinner."

All of this must still respect the label schema above.
...
- Each line must have exactly two keys:
  ...
  - "ner_tags": list of integers (same length as tokens)
- Use proper BIO rules:
  - Every I- tag must be immediately preceded by the corresponding B- tag of the same entity type.
  - No orphan I-tags.
- Entities can be 1–4 tokens long.
- Multi-word entities must use B- then I- tags:
  - "Sunday brunch" → [15, 16]
  - "first steps" → [17, 18]
  - "grandma's necklace" → [19, 20]
  - "birthday party" → [11, 12]
- Possessives:
...
### VARIETY & COVERAGE REQUIREMENTS

Across the generated samples:

- ALL 10 entity types (PERSON, KINSHIP, NICKNAME, PET, HOME_LOC, FAMILY_EVENT,
  ROUTINE, TRADITION, MILESTONE, HEIRLOOM) must appear many times.
- Include both Western and Indian family setups (as described in the user prompt).
- Include multi-token entities regularly:
  - Full names ("Emma Grace Parker"), multi-word locations ("backyard garden corner"),
    multi-word events ("birthday party", "graduation ceremony"), etc.
- Include pet-related sentences often (dogs, cats, fish, birds, rabbits, etc.).
- Include a healthy mix of:
  - routines (school run, dinner time),
  - traditions (Sunday brunch, Diwali celebration),
  - milestones (first steps, lost first tooth),
  - heirlooms (grandma's necklace, dad's old watch).

### OUTPUT FORMAT EXAMPLES (FOLLOW EXACTLY)
...
Now generate the requested number of samples in JSONL format."""
