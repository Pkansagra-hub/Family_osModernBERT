...
def binarize_labels(example):
    ...
    is_profane = (
        example.get("obscene", 0) >= 0.3  # Lower threshold for profanity
        and example.get("toxicity", 0) >= THRESHOLD
    )
...
def create_curated_dataset(
    dataset, output_path, split_name, max_samples=None, oversample_rare=True
):
...
def main():
    ...
    val_samples = create_curated_dataset(
        dataset["validation"], OUTPUT_DIR, "validation", max_samples=20000, oversample_rare=False
    )
    ...
    metadata = {
        ...
        "curated_distribution": {label: curated_counts[label] for label in OUTPUT_LABELS},
