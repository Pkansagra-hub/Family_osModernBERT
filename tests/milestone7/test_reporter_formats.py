...
def _fake_result() -> BenchmarkRunResult:
    summary = BenchmarkSummary(total=2, passed=1, failed=1, skipped=0, errored=0, duration_sec=1.25, warned=0, info=0)
    suite = SuiteResult(
        suite_name="api",
        results=[
...
class TestReporterFormats:
    ...
    def test_markdown_contains_tables(self):
        ...
        assert "| Total | Passed | Failed | Skipped | Errored | Duration (s) |" in md
        assert "## Results by suite" in md
        assert "### api" in md
        assert "| Status | Severity | Name |" in md
